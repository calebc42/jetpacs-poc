...
(defun jetpacs-components--trailing-node (trailing)
  "Return TRAILING as the one node `jetpacs.list_item' accepts.
A node passes through; a list of nodes becomes a centered `jetpacs-row'
of them (one node alone stays bare); nil stays nil.  Anything else is
left for `jetpacs-component-list-item' to refuse."
  (cond
   ((null trailing) nil)
   ((jetpacs-root-node-p trailing) trailing)
   ((and (listp trailing) (cl-every #'jetpacs-root-node-p trailing))
    (if (cdr trailing)
        (apply #'jetpacs-row (append trailing (list :align "center" :spacing 8)))
      (car trailing)))
   (t trailing)))
...
composition, which every receiver can draw.  TRAILING is one node or a
list of nodes either way: the canonical card lays a list out itself,
and the flat node, whose `trailing' member is typed as one node, takes
the list composed into a `jetpacs-row' here -- so a host row that ends
in a badge and a chevron builds on both kinds of receiver.  KEY and
PADDING ride the result as universal attributes, so a caller places the
row in a `lazy_column' the same way either way."
         ...
         (row
          (if (and
             ...
             (jetpacs-component-list-item
              ...
              :subtitle-max-lines subtitle-max-lines
              :leading leading
              :trailing (jetpacs-components--trailing-node trailing)
