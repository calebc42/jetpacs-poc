...
(ert-deftest jetpacs-components-list-item-composes-a-trailing-list ()
  "A trailing LIST builds on both receivers: the flat node's one typed
`trailing' member takes the list as a row, the canonical card lays it
out itself.  The host row (`jetpacs-chrome-row') promises one node or a
list, and the Material 3 catalog ends every example row in a badge plus
a chevron -- which used to fail every component screen on a receiver
that advertises `jetpacs.list_item'."
  (let ((two (list (jetpacs-icon "star") (jetpacs-icon "chevron_right"))))
    (cl-letf (((symbol-function 'jetpacs-node-advertised-p) (lambda (&rest _) t)))
      (let* ((row (jetpacs-components-list-item "Filled" :trailing two))
             (trailing (plist-get row :trailing)))
        (should (equal (plist-get row :t) "jetpacs.list_item"))
        (should (equal (plist-get trailing :t) "row"))
        (should (equal (mapcar (lambda (c) (plist-get c :name))
                               (append (plist-get trailing :children) nil))
                       '("star" "chevron_right"))))
      ;; One node in a list is that node; a bare node is untouched.
      (should (equal (plist-get (plist-get (jetpacs-components-list-item
                                            "One" :trailing (list (jetpacs-icon "star")))
                                           :trailing)
                                :t)
                     "icon"))
      (should (equal (plist-get (plist-get (jetpacs-components-list-item
                                            "Bare" :trailing (jetpacs-icon "star"))
                                           :trailing)
                                :t)
                     "icon"))
      ;; Anything that is not a node or a list of nodes is still refused.
      (should-error (jetpacs-components-list-item "Bad" :trailing "text"))
      (should-error (jetpacs-components-list-item
                     "Bad" :trailing (list (jetpacs-icon "star") "text"))))
    (cl-letf (((symbol-function 'jetpacs-node-advertised-p) (lambda (&rest _) nil)))
      (let ((row (jetpacs-components-list-item "Filled" :trailing two)))
        (should (equal (plist-get row :t) "card"))
        (should (string-search "chevron_right" (jetpacs-node->canonical-json row)))))))
