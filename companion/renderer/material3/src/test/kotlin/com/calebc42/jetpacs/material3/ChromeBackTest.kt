...
class ChromeBackTest {
    ...
    @Test
    fun aPresentedScreenKeepsItsBackArrowThroughTheWrapper() {
        val bar = """"top_bar":{"t":"row","children":[
            {"t":"icon_button","icon":"arrow_back",
             "on_tap":{"builtin":"view.switch","view":"home"}}]}"""
        val scaffold = """{"t":"scaffold","body":{"t":"text","text":"b"},$bar}"""

        // One wrapper: the design scope an app presents its screen inside.
        val wrapped = """{"t":"jetpacs.design_scope","tokens":{},"styles":{},
                          "children":[$scaffold]}"""
        assertEquals(viewSwitch("home"), chromeBackDescriptor(spec(wrapped)))

        // Nested wrappers stay within the host's own bound.
        val nested = """{"t":"jetpacs.design_scope","tokens":{},"styles":{},"children":[
                           {"t":"jetpacs.design_scope","tokens":{},"styles":{},
                            "children":[$scaffold]}]}"""
        assertEquals(viewSwitch("home"), chromeBackDescriptor(spec(nested)))

        // A wrapper holding more than the presented screen is not one.
        val twoChildren = """{"t":"jetpacs.design_scope","tokens":{},"styles":{},
                              "children":[$scaffold,{"t":"text","text":"x"}]}"""
        assertNull(chromeBackDescriptor(spec(twoChildren)))

        // Past the bound the gesture falls back to the system default.
        var deep = scaffold
        repeat(5) {
            deep = """{"t":"jetpacs.design_scope","tokens":{},"styles":{},
                       "children":[$deep]}"""
        }
        assertNull(chromeBackDescriptor(spec(deep)))
    }
}
