...
class ThemeModelTest {
    ...
    @Test
    fun scopeRolesTakeTheSameDerivationAsThePushedPalette() {
        val base = lightColorScheme()
        val roles = com.calebc42.ebp.renderer.compose.ComposeThemeRoles(
            primary = Color(0xFF8A5A2B), onPrimary = Color(0xFFFFFAF2),
            secondary = Color(0xFF6C6356), onSecondary = Color(0xFFFBF8F1),
            error = Color(0xFFA5462F), onError = Color(0xFFFBF8F1),
            background = Color(0xFFF3EDE1), onBackground = Color(0xFF2A251F),
            surface = Color(0xFFFBF8F1), onSurface = Color(0xFF2A251F),
            outline = Color(0xFFD3C9B6), success = Color(0xFF4F7A3A),
            warning = Color(0xFFA9761D),
        )
        val fromRoles = buildColorScheme(roles::byWireName, base)
        val fromPayload = buildColorScheme(
            buildJsonObject {
                put("primary", "#8A5A2B"); put("on_primary", "#FFFAF2")
                put("secondary", "#6C6356"); put("on_secondary", "#FBF8F1")
                put("error", "#A5462F"); put("on_error", "#FBF8F1")
                put("background", "#F3EDE1"); put("on_background", "#2A251F")
                put("surface", "#FBF8F1"); put("on_surface", "#2A251F")
                put("outline", "#D3C9B6"); put("success", "#4F7A3A")
                put("warning", "#A9761D")
            },
            base,
        )
        // ColorScheme has reference equality; compare the derived roles.
        assertEquals(fromPayload.primary, fromRoles.primary)
        assertEquals(fromPayload.primaryContainer, fromRoles.primaryContainer)
        assertEquals(fromPayload.surfaceVariant, fromRoles.surfaceVariant)
        assertEquals(fromPayload.surfaceContainerHigh, fromRoles.surfaceContainerHigh)
        assertEquals(fromPayload.outlineVariant, fromRoles.outlineVariant)
        assertEquals(fromPayload.onBackground, fromRoles.onBackground)
        assertEquals(Color(0xFFF3EDE1), fromRoles.background)
        assertEquals(Color(0xFF8A5A2B), fromRoles.primary)
        val extended = buildExtendedColors(roles::byWireName, dark = false)
        assertEquals(Color(0xFF4F7A3A), extended.success)
        assertEquals(null, roles.byWireName("tint"))
    }
