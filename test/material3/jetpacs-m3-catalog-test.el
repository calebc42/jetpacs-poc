...
(defmacro jetpacs-m3-test--with-clean-projections (&rest body)
  "Run BODY with isolated Preview-default inspection state."
  (declare (indent 0))
  `(let ((jetpacs-m3-inspection-root-mode "preview")
         (jetpacs-m3-inspection-component-modes
...
(ert-deftest jetpacs-m3-every-screen-builds-with-the-components-package ()
  "Each screen still builds once `jetpacs-components' is loaded.
The device init loads that package, so on the tablet every host row is
`jetpacs-components-list-item' -- the flat `jetpacs.list_item' with no
client to say otherwise -- rather than the canonical card the suite
otherwise builds.  The example rows end in a badge and a chevron, a
LIST in the row's trailing slot, which the flat node once refused:
every component screen on the device degraded to \"failed to build\"
while this suite stayed green."
  (require 'jetpacs-components)
  (should (fboundp 'jetpacs-components-list-item))
  (let ((jetpacs-m3-mark-expressive t))
    (dolist (cell (jetpacs-m3-test--screens))
      (let ((node (cdr cell)))
        (should (jetpacs-root-node-p node))
        (should-not (string-search "failed to build"
...
(ert-deftest jetpacs-m3-material-version-matches-the-toml ()
  "THE UNANIMOUS-UPDATE MECHANISM.  Material is this renderer's design
language and its version has ONE source of truth --
...
(ert-deftest jetpacs-m3-catalog-has-jetpacs-material-branding ()
  ...
  (should (equal jetpacs-m3-identity
                 "Jetpacs Material 3 Catalog — authored in Elisp")))
...
(ert-deftest jetpacs-m3-example-screen-carries-the-builder-docstring ()
  "The Source projection says what M3 calls this AND what the elisp says.
Upstream's `:description' is product copy; the builder's docstring is
the only part written by whoever actually drew it."
  (jetpacs-m3-test--with-clean-projections
    (let* ((component (jetpacs-m3-component "switches"))
           (_ (puthash (jetpacs-m3-example-screen-id "switches" 0)
                       "source" jetpacs-m3-inspection-example-modes))
           (texts (jetpacs-m3-test--texts
                   (jetpacs-m3-example-screen component 0 nil))))
      (should (member "Authored Elisp" texts))
      (should (member "Builder documentation" texts))
      (should (cl-some (lambda (s) (string-match-p "Upstream SwitchSample" s))
                       texts)))))

(ert-deftest jetpacs-m3-inspection-selector-exists-at-every-scope ()
  "Every scope exposes one controlled, overflow-safe tab strip."
  (jetpacs-m3-test--with-clean-projections
    (let ((component (jetpacs-m3-component "switches")))
      (dolist (screen (list (jetpacs-m3-home-screen nil)
                            (jetpacs-m3-component-screen component nil)
                            (jetpacs-m3-example-screen component 0 nil)))
        (let ((json (jetpacs-node->canonical-json screen)))
          (should (= 1 (cl-loop with start = 0
                                while (string-match "\"t\":\"tab_selector\""
                                                    json start)
                                count t do (setq start (match-end 0)))))
          (should (string-match-p "Preview" json))
          (should (string-match-p "Visual" json))
          (should (string-match-p "Lisp" json))
          (should (string-match-p "Source" json))
          (should (string-match-p "\"scrollable\":true" json)))))))

(ert-deftest jetpacs-m3-inspection-design-projection-wraps-or-explains ()
  "Design presents the same Preview under the active design profile."
  (jetpacs-m3-test--with-clean-projections
    (should (equal (aref jetpacs-m3-inspection-modes 4) "design"))
    (should (= (length jetpacs-m3-inspection-modes)
               (length jetpacs-m3-inspection-mode-labels)))
    (let ((component (jetpacs-m3-component "buttons")))
      ;; Advertised: the whole screen sits inside one design scope whose
      ;; body still carries the Material button examples unchanged.
      (cl-letf (((symbol-function 'jetpacs-extension-advertised-p)
                 (lambda (extension target)
                   (and (equal extension "jetpacs.design") (eq target :app)))))
        (puthash (jetpacs-m3-example-screen-id "buttons" 0) "design"
                 jetpacs-m3-inspection-example-modes)
        (let* ((screen (jetpacs-m3-example-screen component 0 nil))
               (json (jetpacs-node->canonical-json screen)))
          (should (equal (plist-get screen :t) "jetpacs.design_scope"))
          (should (string-search "\"t\":\"button\"" json))
          (should (string-search "m3catalog.presentation" json))
          (should (jetpacs-check-profile screen 'app))))
      ;; Not advertised: Preview with an explanatory caption, no scope.
      (cl-letf (((symbol-function 'jetpacs-extension-advertised-p)
                 (lambda (&rest _) nil)))
        (let* ((screen (jetpacs-m3-example-screen component 0 nil))
               (json (jetpacs-node->canonical-json screen)))
          (should (equal (plist-get screen :t) "scaffold"))
          (should-not (string-search "jetpacs.design_scope" json))
          (should (string-search "not advertised" json)))))))

(ert-deftest jetpacs-m3-inspection-action-is-strict-and-isolated ()
  "Projection selection validates addresses and cannot leak across scopes."
  (jetpacs-m3-test--with-clean-projections
    (cl-letf (((symbol-function 'jetpacs-flow-continue) (lambda (_fn) nil)))
      (should (eq 'rejected
                  (jetpacs-m3-inspection--on-presentation
                   '(:scope "root" :value 9) nil)))
      (should (eq 'stale
                  (jetpacs-m3-inspection--on-presentation
                   '(:scope "component" :component "missing" :value 1) nil)))
      (should (eq 'accepted
                  (jetpacs-m3-inspection--on-presentation
                   '(:scope "component" :component "buttons" :value 1) nil)))
      (should (equal "visual" (jetpacs-m3-inspection--component-mode "buttons")))
      (should (equal "preview" (jetpacs-m3-inspection--component-mode "switches")))
      (should (eq 'accepted
                  (jetpacs-m3--on-source
                   '(:component "switches" :index 0) nil)))
      (should (equal "source" (jetpacs-m3-inspection--example-mode
                               "switches" 0)))
      (should (equal "preview" jetpacs-m3-inspection-root-mode)))))

(ert-deftest jetpacs-m3-example-projections-mount-only-active-resources ()
  "Preview alone owns sample slots; Lisp alone owns the synchronized editor."
  (jetpacs-m3-test--with-clean-projections
    (let* ((component (jetpacs-m3-component "bottom-app-bar"))
           (screen-id (jetpacs-m3-example-screen-id "bottom-app-bar" 0)))
      (dolist (mode '("preview" "visual" "lisp" "source"))
        (puthash screen-id mode jetpacs-m3-inspection-example-modes)
        (let ((json (jetpacs-node->canonical-json
                     (jetpacs-m3-example-screen component 0 nil))))
          (if (equal mode "preview")
              (should (string-match-p "\"bottom_bar\":" json))
            (should-not (string-match-p "\"bottom_bar\":" json)))
          (if (equal mode "lisp")
              (progn
                (should (string-match-p "\"t\":\"editor\"" json))
                (should (string-match-p "\"action\":\"m3catalog.repl\"" json)))
            (should-not (string-match-p "\"t\":\"editor\"" json))))))))

(ert-deftest jetpacs-m3-every-inspection-projection-builds ()
  "All four projections canonicalize and remain in the app profile."
  (jetpacs-m3-test--with-clean-projections
    (dolist (mode '("preview" "visual" "lisp" "source"))
      (setq jetpacs-m3-inspection-root-mode mode)
      (let ((home (jetpacs-m3-home-screen nil)))
        (should (jetpacs-check-profile home 'app))
        (should (stringp (jetpacs-node->canonical-json home))))
      (dolist (component jetpacs-m3-components)
        (let ((id (plist-get component :id)))
          (puthash id mode jetpacs-m3-inspection-component-modes)
          (let ((screen (jetpacs-m3-component-screen component nil)))
            (should (jetpacs-check-profile screen 'app))
            (should (stringp (jetpacs-node->canonical-json screen))))
          (cl-loop for _example in (plist-get component :examples)
                   for index from 0
                   do (puthash (jetpacs-m3-example-screen-id id index)
                               mode jetpacs-m3-inspection-example-modes)
                      (let ((screen
                             (jetpacs-m3-example-screen component index nil)))
                        (should (jetpacs-check-profile screen 'app))
                        (should (stringp
