...
| **RF-2c EXIT** | `:wire` platform-freedom compiler-enforced | **MET 2026-08-01** — executed as the corrected H1–H8 ladder per [PLAN-rf2c-hoist.md](PLAN-rf2c-hoist.md) (`177e6e6`), which supersedes §3.2/§3.3 where they conflict (four hard runbook defects — see the dated correction notes there). Branch `rf-2c`, all steps green checkpoints: H1 `0881a7e` (7 pure moves + WireLimits/taxonomy + ContentInvalid extractions, Opus, zero deviations); H2 `6b5a2d8` (TextUnits: utf8Size + SIX codepoint helpers oracle-pinned incl. lone surrogates; **measured correction**: on the JVM `encodeToByteArray()` substitutes `?` = 1 byte for a lone surrogate, identical to the old tape — U+FFFD/3-byte is Kotlin/Native's encoder, so §3.2's caveat was wrong as stated and the delta is per-platform, dormant); H3 pin `5bd031f` (compaction self-overlap past 64 KiB — the branch had zero coverage) + H3 `34767de` (FrameCodec → ByteArray/commonMain: hand-rolled total ISO-8859-1 header map, `decodeToString(throwOnInvalidSequence)`, `copyInto`; **Defect D whole in one commit** — encodeFrame + the five emit-coupled byte tapes flip together through `utf8Size()`); H4 `3bfe3fb` (Platform.kt seven expects + jvm actuals; Auth common, US_ASCII→UTF-8 alignment recorded, KAT byte-exact); H5 `44b393c` (validator tier + **ImageGuards split**: common string policy / jvm NetGuards; `Math.floor` ×9 was grep-invisible, counted 9→0); H6 `460ef1f`/`d70e47d`/`daf012a` (backing splits + shared `writeJsonAtomic`, PersistenceCompat 9/9; kotlinx-datetime **0.6.2** + DurableQueue 16 locks + 2 flagged admit micro-fixes; stores 8+6 locks, SurfaceStore stays lockless by design); H7 `f22bc43` (trigger stack: kotlinx civil time day-wrap pinned by the wrapped-window fixtures, **AtomicRef CAS kept** — the runbook's @Volatile demotion was wrong, TFS detach CAS is load-bearing; LD-16 callout stays outside the lock; one flagged deviation: kotlinx `LocalTime.compareTo` is strictly non-null where java's took a platform type — `before!!` made the branch-1 invariant explicit); H8 `8092bb5` (CompanionEngine: sweep 24→**25** — `dispatchDialogAction`'s missing lock closed, ratified+flagged; catchingStackOverflow seam; 5 codepoint compat swaps); stragglers `c7ba0dd` (22 imports orphaned by the split) + `21a093d` (`kotlin.jvm.JvmInline` explicit — kotlin.jvm.* is only a default import on JVM compilations; caught by the metadata pre-flight). **Exit gates from clean (49/49 tasks executed)**: G-wire **40 suites / 361 tests** (353 + 7 TextUnits + the compaction pin), G-app 26 + APK, G-spec, G-elisp, I1. **Lock audit exact: `@Synchronized` 0; `lock.withLock` 61** = CE 25 + DQ 16 + Reminder 8 + Trigger 6 + TFS 6 (59 + TFS block + the ratified fix). **jvmMain == exactly 7 files** (Platform.jvm, NetGuards, JsonFileIo, 4×File backings — §3.3 said six; the ImageGuards split adds one). **Purity part 2: `:wire:compileKotlinLinuxX64` green** on scratch actuals (never committed) — commonMain is compiler-proven pure under a non-JVM frontend; expect/actual Beta warnings are KT-61573, warning-only. Push/PR/CI + device smoke are the user's |
...
> **Correction (2026-08-01, RF-2c as executed).** Two claims above were falsified at H2/H6: (i) the lone-surrogate caveat is wrong for the JVM — `encodeToByteArray()` delegates to java's REPLACE-mode encoder, so a lone surrogate is `?` = **1 byte, identical to the retired tape**; the U+FFFD/3-byte substitution is Kotlin/Native's encoder, so the delta is per-**platform** (dormant until a native target lands), not JVM-vs-JVM. The durable invariant is the single tape: every gate measures through `TextUnits.utf8Size()`, which IS `encodeToByteArray().size`, so measure == emit on any platform whatever its substitution (`TextUnitsTest.utf8SizeMatchesTheJvmTapeIncludingLoneSurrogates` pins the JVM figure; no jvm-actual byte counter was needed). (ii) The `DateTimeException` retarget clause was a no-op — zero such catches exist anywhere in `:wire` (also recorded in LIBRARY-LEDGER). Two additions the section lacked: TextUnits needed **six** helpers, not three (+`charCountCompat`, `codePointsOf`, `stringFromCodePoints` for `EditorSession.diff`), and the seam gained `AtomicRef` + `catchingStackOverflow` expects (see the §3.3 correction).
...
> **Correction (2026-08-01, RF-2c as executed).** Line-verified exploration found four hard defects in this order, and [PLAN-rf2c-hoist.md](PLAN-rf2c-hoist.md) — the executed H1–H8 ladder — supersedes it. (A) `WireLimits` + the frame-taxonomy exceptions live in the H3 file but are consumed by H1/H2 files: both extract to commonMain **at H1** (with `ContentInvalid` out of SpecValidator). (B) The trigger stack depends on validators/stores, so it moves **after** them (H7), not before; `TriggerValidator` was a verbatim pure move. (C) `SpecValidator` (common-bound) calls into `ImageGuards`' InetAddress half, so ImageGuards **splits**: pure string policy → commonMain, network/decode half → new jvmMain `NetGuards` — end-state jvmMain is **seven** files (+ `JsonFileIo.kt` for the factored `writeJsonAtomic`, − the never-split `ImageGuards.kt` as listed above). (D) `encodeFrame`'s tape flip and the five emit-coupled byte gates must land in **one commit** through a single `String.utf8Size()` helper or measure==emit breaks mid-ladder. Also: the parenthetical AtomicReference demotion here was **wrong** — `TriggerFiringService.detach`'s CAS is load-bearing against a documented lost-update race; it ported to an `AtomicRef` expect/actual with real `compareAndSet`. The `@Synchronized` ground truth was 59 + one `synchronized(this)` block (TFS, LD-16 callback deliberately outside) + one ratified fix (`dispatchDialogAction` had no lock; closed at H8, flagged) = **61 `withLock` sites**.
