...
### GR-8d — periphery re-homing (**OR-2: flagged; needs the ruling**)
With GR-7a's funnel replacement landed, ef + gallery's last Glasspane couplings are gone. **Recommendation:** ef → `emacs/apps/ef-themes/` (org-free, atop the promoted `jetpacs-theme-picker`, Settings-satellite link order 81, `:any-surface` rationales re-recorded verbatim); gallery → foundation demo tier registered from device/init.el's demo block. Until ruled, both stay app-side and green. srs stays app-side regardless (first-class Review destination, notes coupling; its four funnels are now mode-neutral or foundation).

**Residual `:any-surface` audit (executable regardless of OR-2's timing):** per-verb re-justification against the S4 sanctioned-guest gate, recorded at the registration site — **the 4 `demo.gallery.*` verbs** (gallery-el:259-270; their current Settings-satellite rationale is exactly what a re-home invalidates: either re-record it verbatim under the new registration site, or drop `:any-surface` where S4 delegation from the satellite screen now covers the entry path — the same test `settings.agenda.edit`/`.delete` already passed at dec2830); **`glasspane.settings.open`** gets its opener rationale recorded explicitly (taps arrive before any guest screen exists) rather than riding "unchanged"; the 7 `ef.*` verbs re-record theirs with the ef move.
...
| glasspane-ef.el | survives; re-home **OR-2 flagged** | org-free; recommendation: `emacs/apps/ef-themes/` Settings-satellite |
| glasspane-gallery.el | survives; re-home **OR-2 flagged** | recommendation: foundation demo tier |
