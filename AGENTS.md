...
This directory is an umbrella, not one homogeneous source tree. Several child
directories are separate Git repositories or workspace projects. A root
`git status` does not describe their internal changes.
...
1. `ebp-poc/ebp/SPEC.md` is the normative EBP POC authority. Its §2.2 explicitly
...
|---|---|
| EBP spec/contract/goldens | `cd ebp-poc/ebp && python3 validate.py` plus every affected implementation conformance test |
| Jetpacs Elisp foundation | Targeted ERT, warning-as-error byte compilation, then `jetpacs/test/run-tests.sh` for broad changes |
| Companion Kotlin/Android | Owning module unit tests; for broad changes run `cd jetpacs/companion && ./gradlew testDebugUnitTest assembleDebug` |
...
jetpacs-platform-tools/build/install/jetpacs-platform-tools/bin/jetpacs-platform-tools \
  --root /home/calebc42/workspace/jetpacs/jetpacs-poc \
