...
## Active checkout sources and trusted launch

The flattened Jetpacs workspace is `/home/calebc42/workspace/jetpacs-poc` and
the seven active app checkouts are its named siblings under
`/home/calebc42/workspace`: `ebp`, `ebp.el`, `ebp-kmp`, `ebp-compose`,
`ebp-org`, `glasspane`, `jetpacs-authoring`. Set
`JETPACS_REPOSITORIES_ROOT=/home/calebc42/workspace` when the process cannot
derive that parent. The platform source is `emacs/` and the active contract is
`ebp/contract.json`.

`JETPACS_TRUSTED_APPLET=glasspane/glasspane.el` is accepted only by the
explicit trusted launcher. It is resolved canonically inside the allowlisted
`glasspane` checkout; parent segments, absolute paths outside an allowlisted
checkout, and symlink escapes are rejected. External package references,
including `org/`, remain inspection-only and are never executable.
