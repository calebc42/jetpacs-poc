...
## Checkout and package sources have separate authorities

The flattened workspace is `/home/calebc42/workspace/jetpacs-poc`. Its active
app sources live in the exact seven sibling checkout directories below
`/home/calebc42/workspace`, exposed as their directory names (`ebp/`,
`ebp.el/`, `ebp-kmp/`, `ebp-compose/`, `ebp-org/`, `glasspane/`,
`jetpacs-authoring/`). Set `JETPACS_REPOSITORIES_ROOT` to that parent
when launching outside the standard layout. If unset, tooling checks only the
workspace's parent and grandparent for those exact names.

The canonical Jetpacs platform source is `emacs/` inside the flattened
workspace. The active EBP contract is `ebp/contract.json`. Checkout sources
are static inspection inputs and explicitly allowlisted trusted applet roots;
they are not merged with package reference roots.
...
`JETPACS_TRUSTED_APPLET`, resolves that `.el` file inside an allowlisted
checkout (for example `glasspane/glasspane.el`), rejects parent-segment and
symlink escapes, loads the real Jetpacs runtime and applet, and only then
advertises live inventory and render-check tools. Read-only package roots,
including external Org, can never become executable applet targets. Loading an
applet executes arbitrary Elisp; use that mode only for code you trust.
