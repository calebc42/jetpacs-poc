...
expansion, file writing, process, shell, or network tool. It can expose
explicitly allowlisted package source trees through separate read-only
namespaces without widening workspace paths. Org is the built-in example and
appears as `org/`; any package can use the same mechanism.
...
```sh
JETPACS_WORKSPACE=/path/to/jetpacs-poc \
JETPACS_ORG_SOURCE=/path/to/emacs/lisp/org \
  emacs -Q --batch -l /path/to/jetpacs-poc/jetpacs-applet-mcp/jetpacs-applet-mcp.el
```
  ...
  "mcpServers": {
    "jetpacs-applets": {
      ...
      "args": [
        ...
        "--batch",
        "-l",
        "/path/to/jetpacs-poc/jetpacs-applet-mcp/jetpacs-applet-mcp.el"
      ],
      "env": {
        "JETPACS_WORKSPACE": "/path/to/jetpacs-poc",
        "JETPACS_ORG_SOURCE": "/path/to/emacs/lisp/org"
...
When the server lives in this checkout, it derives the workspace from its own
path. It also discovers this repository layout's Org checkout at
`../../emacs/emacs/lisp/org`; the environment variables remain the portable,
...
JETPACS_REFERENCE_ROOTS='{"magit":"/src/magit/lisp","denote":"/src/denote"}' \
  emacs -Q --batch \
  -l /path/to/jetpacs-poc/jetpacs-applet-mcp/jetpacs-applet-mcp.el
```
...
JETPACS_WORKSPACE=/path/to/jetpacs-poc \
JETPACS_ORG_SOURCE=/path/to/emacs/lisp/org \
JETPACS_TRUSTED_APPLET=jetpacs-applet-mcp/examples/hello/hello.el \
  emacs -Q --batch -L /path/to/ebp.el/lisp \
  -l /path/to/jetpacs-poc/jetpacs-applet-mcp/jetpacs-applet-mcp-runtime.el
```
