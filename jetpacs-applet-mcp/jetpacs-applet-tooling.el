...
(defcustom jetpacs-applet-tooling-source-directories
  '("jetpacs/emacs" "glasspane" "jetpacs-applet-mcp")
...
source reconciliation.  Discovery recognizes `emacs' and `jetpacs/emacs';
...
Discovery recognizes the sibling Emacs checkout at
`../../emacs/emacs/lisp/org' relative to the Jetpacs workspace.  This is a
...
(defun jetpacs-applet-tooling--legacy-org-candidate ()
  ...
  (let* ((configured (or (getenv "JETPACS_ORG_SOURCE")
         ...
         (discovered (expand-file-name
                      "../../emacs/emacs/lisp/org"
                      (jetpacs-applet-tooling-workspace-root))))
    (or configured (and (file-directory-p discovered) discovered))))
...
Paths beginning with a configured NAMESPACE/ resolve only under that explicit
read-only reference root; all other paths resolve under the workspace."
  (unless (and (stringp path) (not (string-empty-p path)))
    (error "Jetpacs Applet Tooling: path must be a non-empty string"))
  (let* ((reference (jetpacs-applet-tooling-reference-path-p path))
         (root (if reference
                   (cdr reference)
                 (jetpacs-applet-tooling-workspace-root)))
         (relative (if reference
                       (substring path (1+ (length (car reference))))
                     path))
         (expanded (expand-file-name relative root)))
    (unless (file-exists-p expanded)
      (error "Jetpacs Applet Tooling: file does not exist: %s" path))
    (let ((true (file-truename expanded)))
      (unless (file-in-directory-p true root)
...
(defun jetpacs-applet-tooling--source-files ()
  ...
  (let (files)
    (dolist (relative jetpacs-applet-tooling-source-directories)
      (let ((dir (expand-file-name relative (jetpacs-applet-tooling-workspace-root))))
        (when (file-directory-p dir)
          (dolist (file (directory-files-recursively dir (rx ".el" eos)))
                ...
                (let* ((start (point))
                  ...
                  (push (append
                         ...
                         (list :form form
                               ...
                               :path (jetpacs-applet-tooling--relative file)
                               :line line))
                        records))))
...
(defun jetpacs-applet-tooling--contract-file ()
  ...
  (let* ((root (jetpacs-applet-tooling-workspace-root))
         (relative "ebp-poc/ebp/contract.json"))
    (unless (file-regular-p (expand-file-name relative root))
      (error "Jetpacs Applet Tooling: no active EBP contract.json found"))
    (jetpacs-applet-tooling-resolve-file relative)))
...
(defun jetpacs-applet-tooling--applet-files (path)
  ...
  (let* ((root (jetpacs-applet-tooling-workspace-root))
    ...
    (let ((true (file-truename expanded)))
      (unless (or (equal (file-name-as-directory true) root)
                  (file-in-directory-p true root))
        (error "Jetpacs Applet Tooling: applet path leaves the workspace: %s" path))
      (cond
       ((file-regular-p true)
                 ...
                 (mapcar
                  (lambda (file)
                    (when-let* ((canonical (file-truename file))
                                ((file-in-directory-p
                                  canonical (jetpacs-applet-tooling-workspace-root)))
             ...
             ('jetpacs-chrome-define-root
              (push (list :surface (jetpacs-applet-tooling--literal-name (nth 1 node) literals)
                          ...
                          :line (jetpacs-applet-tooling--registration-line
                                 ...
                                 (plist-get record :line)))
                    roots)))))))))
    (list :path (jetpacs-applet-tooling--relative file)
          :records records
...
(defun jetpacs-applet-tooling--applet-manifest-data (path)
  ...
  (let* ((files (jetpacs-applet-tooling--applet-files path))
         (canonical-target
          (file-truename
           (expand-file-name path (jetpacs-applet-tooling-workspace-root))))
         (canonical-path
...
prompt/blocking calls inside registered handlers."
  (let* ((files (jetpacs-applet-tooling--applet-files path))
         ...
         issues)
    (dolist (scan scans)
      (let* ((file (expand-file-name (plist-get scan :path)
                                     (jetpacs-applet-tooling-workspace-root)))
