...
**Jetpacs Material 3 Catalog** (short dock label: **Material 3**). The
established `jetpacs-m3-*` symbols and `m3catalog.*` wire verbs are the
reusable library and stable wire identities Jetpacs consumes; persisted
...
`jetpacs.material3` renderer extension. Jetpacs itself targets EBP semantics
...
**Jetpacs Components** app in the `jetpacs-component-catalog/` module. New
Jetpacs-owned controls do not enter this Material inventory, and the stable
`jetpacs-m3-*` library remains the implementation Jetpacs consumes.

```
emacs/apps/m3-catalog/jetpacs-m3-catalog.el           entry point: requires everything, registers the app
...
test/material3/jetpacs-m3-catalog-test.el      the exit gate (builds every screen)
...
* the same gate requires the catalog's `jetpacs.material3` app declaration;
* `test/material3/jetpacs-m3-catalog-test.el` pins the complete inventory, reverse index,
...
`docs/material3/lookup-tables/M3-COMPONENT-LOOKUP.org` is the historical
...
`jetpacs.material3` extension. Carousel, SearchBar, SegmentedButton,
...
6. Stateful nodes (`checkbox`, `switch`, `slider`, `text_input`,
   ...
   `"checkboxes-tristate"`.
7. Icons must exist in `docs/material3/lookup-tables/M3-ICON-REFERENCE.org`
...
`test/material3/jetpacs-m3-catalog-test.el`.

## Node vocabulary cheat-sheet

Full reference: `docs/material3/lookup-tables/WIDGET-REFERENCE.org`; the
...
Color roles (`docs/material3/lookup-tables/COLORS-REFERENCE.org`): `primary`,
