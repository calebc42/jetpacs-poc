...
;; Component modules live beside this entrypoint in both the source tree
;; and the managed distribution. Directly loading the entrypoint must also
;; make those sibling modules discoverable.
(eval-and-compile
  (let* ((here (or load-file-name buffer-file-name))
         (dir (and here (file-name-directory (expand-file-name here)))))
