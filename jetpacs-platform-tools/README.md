...
as `org/…`; it does not broaden workspace traversal. The platform server has
no write, shell, build, network, or arbitrary-evaluation tool.
...
jetpacs-platform-tools --root /path/to/jetpacs-poc summarize jetpacs/emacs/jetpacs-apps.el
...
With no command, or with `serve`, it starts an MCP stdio server. The root is
selected in this order: `--root`, `JETPACS_WORKSPACE`, then parent discovery
from the current directory. Org source is selected by `--org-source`,
